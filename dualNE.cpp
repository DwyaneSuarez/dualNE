...
const int MAX_NUM_USERS = 40000;
const int MAX_NUM_ITEMS = 50000;
const int MAX_DIM = 1050;
int NUM_USERS, NUM_ITEMS;

struct Record
{
    std::vector<int> positiveItems;
    std::set<int> itemSet;
}rec[MAX_NUM_USERS + 1];

std::vector<Pii> sample;
float vecU[MAX_NUM_USERS + 1][MAX_DIM], vecI[MAX_NUM_ITEMS + 1][MAX_DIM];

char network_file[MAX_STRING], embedding_file[MAX_STRING], uu_network[MAX_STRING], ii_network[MAX_STRING], ui_network[MAX_STRING];
...
/* Update embeddings */
void Update(real *vec_u, real *vec_v, real *vec_error, int label, real para)
{
	real x = 0, g;
	for (int c = 0; c != dim; c++) x += vec_u[c] * vec_v[c];
	g = (label - FastSigmoid(x)) * rho * para;
	for (int c = 0; c != dim; c++) vec_error[c] += g * vec_v[c];
	for (int c = 0; c != dim; c++) vec_v[c] += g * vec_u[c];
}

void *TrainSide(void *id)
{
	...
	while (1)
	{
		...
		for (int d = 0; d != num_negative + 1; d++)
		{
			...
			lv = target * dim;
			if (order == 1) Update(&emb_vertex[lu], &emb_vertex[lv], vec_error, label, 0.5);
			if (order == 2) Update(&emb_vertex[lu], &emb_context[lv], vec_error, label, 0.5);
		}
...
void OutputFinal()
{
    FILE *fout = fopen(embedding_file, "w+");
...
// Load embedding vector trained by side information
void LoadVector()
{
    FILE *fside = fopen(embedding_file, "r");
    int num, d;
    fscanf(fside, "%d%d", &num, &d);
    for (int i = 0; i < num; i++)
    {
        int id;
        fscanf(fside, "%d", &id);
        ...
        {
            if (id <= NUM_USERS)
                fscanf(fside, "%f", &vecU[id][j]);
            else
                fscanf(fside, "%f", &vecI[id - NUM_USERS][j]);
        }
    }
    fclose(fside);
}
...
void UpdateBPR(int u, int i, int j)
{

    float gradientU[dim], gradientI[dim], gradientJ[dim];
    std::fill(gradientU, gradientU + dim, 0.0);
    std::fill(gradientI, gradientI + dim, 0.0);
    std::fill(gradientJ, gradientJ + dim, 0.0);
...
void Train() {
	...
	boost::thread *pt1 = new boost::thread[num_threads];
	boost::thread *pt2 = new boost::thread[num_threads];
	...
	InitHashTable();
	strcpy(network_file, uu_network);
	ReadData();
	InitAliasTable();
	InitVector();
	InitNegTable();
	...
	clock_t start = clock();
	printf("--------------------------------\n");
	for (a = 0; a < num_threads; a++) pt[a] = boost::thread(TrainSide, (void *)a);
	...
	InitHashTable();
	strcpy(network_file, ii_network);
	ReadData();
	InitAliasTable();
	InitVector();
	InitNegTable();
	...
	start = clock();
	printf("--------------------------------\n");
	for (a = 0; a < num_threads; a++) pt1[a] = boost::thread(TrainSide, (void *)a);
	for (a = 0; a < num_threads; a++) pt1[a].join();
	printf("\n");
	finish = clock();
	printf("Total time: %lf\n", (double)(finish - start) / CLOCKS_PER_SEC);
	Output();
	...
	start = clock();
	printf("--------------------------------\n");
	for (a = 0; a < num_threads; a++) pt2[a] = boost::thread(TrainUI, (void *)a);
	for (a = 0; a < num_threads; a++) pt2[a].join();
	printf("\n");
	finish = clock();
	printf("Total time: %lf\n", (double)(finish - start) / CLOCKS_PER_SEC);
...
int main(int argc, char **argv) {
	...
	if ((i = ArgPos((char *)"-trainUI", argc, argv)) > 0) strcpy(ui_network, argv[i + 1]);
	if ((i = ArgPos((char *)"-trainUU", argc, argv)) > 0) strcpy(uu_network, argv[i + 1]);
	if ((i = ArgPos((char *)"-trainII", argc, argv)) > 0) strcpy(ii_network, argv[i + 1]);
    if ((i = ArgPos((char *)"-num_users", argc, argv)) > 0) NUM_USERS = atoi(argv[i + 1]);
	if ((i = ArgPos((char *)"-num_items", argc, argv)) > 0) NUM_ITEMS = atoi(argv[i + 1]);
